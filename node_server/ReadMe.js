...
// npm install -g nodemon : 소스코드 수정 시 자동으로 서버를 재시작하는 nodemon 모듈 설치

// 2. 위에서 설치된 라이브러리들이 node_modules 폴더에 설치됨 

// 3. 파워쉘(관리자모드실행)에서 nodemon을 이용하여 서버를 실행
// Set-ExecutionPolicy RemoteSigned -Scope CurrentUser

// 4. 메인 폴더에 public 폴더 생성
// public 폴더에는 html, css, js, image 등의 정적 파일(정적자원)을 저장하는 공간이다.
...
//  2) 웹서버가 우리의 요청을 받아서 해당 파일을 웹브라우저에게 랜더링 해준다.
...
// 3. 웹서버 프로그램
//  1) 웹서버 프로그램은 웹서버의 
//  2) node.js의 http 모듈을 사용하여 웹서버 프로그램을 작성할 수 있다.
...
//  3) 웹브라우저는 응답을 받아서 화면에 랜더링한다.

// 5. 요청 Url(uniform resource locator)
//  1) 웹즈라우저 주소창에 https://www.naver.com:8080/id=hong을 입력하면 웹브라우저는 naver.com 서버에 요청을 보낸다.
//  2) 요청 url은 프로토콜, 호스트, 포트, 경로, 퀘리스트링으로 구성된다.
//  3) 프로토콜 : http, https, ftp, file 등의 프로토콜을 사용한다.
//  4) 호스트 : 서버의 도메인 이름이나 ip 주소를 나타낸다.
//  5) 포트 : 서버의 포트 번호를 나타낸다.
//  6) http: 프로토콜은 html문서를 전송하기 위한 프로토콜이다.
...
// 8. http 프로토콜
// 1) http(Hyper Text Transfer Protocol) : 웹브라우저와 웹서버 간에 데이터를 주고받기 위한 프로토콜이다.
...
//    응답은 웹서버에서 요청을 처리한 후 겨로가 페이지를 웹브라우저에 전송하는 것을 말한다.

// 9. DNS(Domain Name System)
// 1) DNS(Domain Name System) : 도메인 이름을 ip 주소로 변환하는 시스템이다.
// 2) dns서버는 도메인 이름을 ip 주소로 변환하고 ip 주소를 도메인 이름으로 변환한다.
// 3) dns서버는 인터넷에 연결된 모든 컴퓨터에 존재한다.

// 10. http 요청 메소드
// 1) 사용자가 웹브라우저 주소창에 www.google.com을 입력하면
// 2) 웹브라우저는 dns서버에 www.google.com을 ip 주소로 변환해달라고 요청한다.
// 3) dns서버는 www.google.com을 ip 주소로 변환한 후 웹브라우저에 전송한다.
// 4) 웹브라우저는 ip 주소를 받아서 행당 서버에 요청을 보낸다.
// 5) 웹브라우저는 웹서버가 보내온 페이지를 화면에 렌더링한다.

// 11. http 요청 메소드
// 1) GET : 데이터를 요청할 때 웹브라우저 주소창에 url에 데이터를 붙여서 전송한다.
...
// 2) 요청 라인 : 요청 메소드, 요청 url, 프로토콜 버전이 포함되어 있다.
// 3) 요청 헤더에는 요청에 대한 정보가 포함되어 있다.
