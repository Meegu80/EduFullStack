...
// fs 모듈 import : 모듈은 파일 처리를 위한 모듈이다.
const fs = require('fs');
...
// 5. 미들웨어 등록
// 5.1. 브라우저에서 form의 데이터를 전송하면 서버에서 JSON 형식으로 데이터를 받을 수 있도록 설정한다.
// 데이터를 받을 수 있도록 설정한다.
app.use(express.json());

// 5.2. 브라우저에서 form의 데이터를 전송하면 서버에서 urlencoded 형식으로 
// 데이터를 받을 수 있도록 설정한다. Form태그에 데이터 입력하고 저장할 떄
// method 속성을 post로 설정하면 그 데이터가 메시지 통의 바디에 포함되어 전송된다.
// 그 데이터를 서버에서 편리하게 그 데이터가 메시지 통의 바디에 포함되어 전송된다.
app.use(express.urlencoded({ extended: true }));

// userData.json 파일에 저장된 데이터를 읽어서 users 변수에 저장한다.
// require() 함수 : node.js에서 제공하는 함수로 파일을 읽어서 자바스크립트 객채로 변환한다.
// 이 함수는 서버 측에서 실행되는 함수이다. 클라이언트 측에서는 사용할 수 없다.
let users = require('./public/userData.json');
...
// req: request 객체 즉, 웹브라우저에서 요청한 정보를담고 있다.
...
// /login 경로로 get방식으로 요청이 오면 login.html 파일을 응답으로 제공한다.
// get 요청은 주로 html 페이지를 요청하는데 사용된다.
...
app.post('/register', (req, res) => {
   ...
   // 기존 사용자 데이터와 중복되는지 확인
    ...
    if(isDuplicated){
        ...
        // 사용자 데이터를 users 배열 추가
        users.push(user);
        // 사용자 데이터를 userData.json 파일에 저장
        // JSON.stringify(users, null, 2): users 객체를 JSON 문자열을 변환한다.
